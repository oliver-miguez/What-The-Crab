...
func _ready() -> void:
	...
	# Registramos el nodo completo en el global para que los aliados puedan acceder a sus funciones
	GlobalPosicionSpawneo.base_enemiga_nodo = self
...
func _on_deteccion_aliados_body_entered(body: Node2D) -> void:
	if body.is_in_group("Aliados"):
		print("Un aliado está atacando la base")

func _on_deteccion_aliados_body_exited(body: Node2D) -> void:
	if body.is_in_group("Aliados"):
		print("Un aliado ha dejado de atacar")
